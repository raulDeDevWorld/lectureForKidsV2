...
console.log(defaultValue)
    ...
    return (
        ...
        <div 
           ...
           { defaultValue === 'Ninguno' ? 'Seleccionar':<li  className='flex items-center  cursor-pointer ' > <img src={defaultValue.img}  className='mr-5 inline w-[30px] border-[1px]' alt="" />{defaultValue.text} </li> }
            <span  className={select ? 'absolute right-5 rotate-[270deg]' :'absolute right-5 rotate-90'}>{'>'}</span>
            <ul 

             className={select ? `absolute h-[100px] overflow-auto left-0 top-[50px] bg-gray-50 outline outline-1 outline-gray-300 text-gray-900 text-[14px] rounded-b-xl focus:ring-blue-500 focus:outline-blue-500 w-full p-3 z-10`: 'hidden' } 
            //  className={`bg-gray-50 border border-gray-300 text-gray-900 text-[12px] rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 
            // dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`} 
            >
                {
                    // console.log(arr)
                    arr.map((i, index)=> <li key={index}  className='flex items-center mb-2 cursor-pointer p-2' onClick={() => handlerUserState(name, i)}> <img src={i.img}  className='mr-5 inline w-[30px] border-[1px]' alt="" />{i.text} </li>)
                }
            </ul>
        </div>
    )
