                            ...
                            : item.label === 'Aprender'
                                ...
                                : item.label === 'Favoritos'
                                    ...
                                    : item.label === 'Perfil'
                                        ...
                                        : false
                    const profileSignedIn = item.label === 'Perfil' && Boolean(user)

                    return (
                        <Link key={item.label} href={item.href} aria-label={item.label}>
                            <span className={`relative flex min-h-14 flex-col items-center justify-center gap-0.5 rounded-[1.25rem] px-1 py-1.5 text-[10px] font-black transition active:scale-95 sm:text-xs ${active ? 'bg-[#A7D8F5] text-[#1F2A44] shadow-inner' : 'text-[#7A8194]'}`}>
