    ...
    return (
        <p className={paragraphClassName}>
            {(tokens || text.split(/(\s+)/)).map((token, index) => {
                ...
                const classNames = [
                    'cursor-pointer rounded-xl px-1.5 py-0.5 transition-colors hover:bg-[#fff1c7]',
                    status === 'matched' ? 'bg-[#dfffe9] text-[#14532d]' : '',
                    status === 'assisted' ? 'bg-[#fff1c7] text-[#8a5a00]' : '',
                    isCurrent ? 'bg-[#4cc9f0] text-[#082f49] underline decoration-[#ff6b6b] decoration-4 underline-offset-4 shadow-[0_4px_0_rgba(8,47,73,0.14)]' : '',
                ].join(' ')
